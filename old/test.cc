...
static const char kEnginePath[] = "/home/francis/Git/KVEngine/engine_race/build/data";
static const char kDumpPath[] = "/home/francis/Git/KVEngine/engine_race/build/dump";
...
void randomRead(Engine* engine, const threadsafe_vector<std::string>& keys, unsigned numRead)
{
    ...
    for (unsigned i = 0; i < numRead; ++i) {
        ...
        std::string val;
        RetCode ret;
        if ((ret = engine->Read(key, &val)) != kSucc) {
            std::cout << "Read key not find " << ret << std::endl;
        }
